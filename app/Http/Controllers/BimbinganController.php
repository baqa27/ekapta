...
class BimbinganController extends Controller
{
    ...
    public function bimbinganMahasiswa()
    {
        ...
        if (!AppHelper::canAccessBimbingan($mahasiswa)) {
            return redirect()->route('pendaftaran.mahasiswa')->with('warning', 'Selesaikan tahap Pendaftaran KP terlebih dahulu. Pendaftaran harus sudah di-ACC oleh Admin.');
        }
        ...
        $bimbinganOffline = $mahasiswa->bimbingans()
            ->where('tipe', 'offline')
    ...
    public function create()
    {
        ...
        if (!$bagian_id) {
            return redirect('bimbingan-mahasiswa')->with('warning', 'Pilih bagian bimbingan terlebih dahulu');
        }

        $bagian = Bagian::findOrFail($bagian_id);

        // Cek apakah bagian sudah di-ACC atau sedang review
        $existingBimbingan = $mahasiswa->bimbingans()->where('bagian_id', $bagian_id)->first();
        if ($existingBimbingan && $existingBimbingan->status == 'diterima') {
            return redirect('bimbingan-mahasiswa')->with('warning', 'Bagian ini sudah di-ACC');
        }
        if ($existingBimbingan && in_array($existingBimbingan->status, ['review', 'revisi'])) {
            return redirect('bimbingan-mahasiswa')->with('warning', 'Bagian ini sedang dalam proses review');
        }
    ...
    public function createManual()
    {
        ...
        if (!$dosenPembimbing || !$dosenPembimbing->is_manual) {
            return redirect('bimbingan-mahasiswa')->with('warning', 'Fitur ini hanya untuk dosen dengan mode bimbingan offline');
        }
        ...
        return view('pages.mahasiswa.bimbingan.create-manual', [
    ...
    public function storeManual(Request $request)
    {
        ...
        return redirect('bimbingan-mahasiswa')->with('success', 'Bimbingan berhasil disubmit. Menunggu validasi dari Prodi.');
    }
    ...
    public function reviewProdi($id)
    {
        ...
        return view('pages.prodi.bimbingan.review', $data);
    }

    public function bimbinganAdmin()
    {
        $mahasiswas = Mahasiswa::with(['bimbingans', 'pendaftarans', 'dosens', 'seminar', 'pengajuans', 'jilid', 'bimbingan_canceleds'])->get();
        return view('pages.admin.bimbingan.bimbingan', [
    ...
    public function bimbinganAdminInput()
    {
        ...
        if($routeName == 'bimbingan.prodi.input' && Auth::guard('prodi')->check()){
    ...
    public function bimbinganAdminInputCreate($dosen_id, $mahasiswa_id)
    {
        ...
        // Ambil semua bagian bimbingan berdasarkan prodi dan tahun masuk mahasiswa
        $prodi = Prodi::where('namaprodi', $mahasiswa->prodi)->first();
        $bagians = $prodi ? $prodi->bagians()->where("tahun_masuk", "LIKE", "%" . $mahasiswa->thmasuk . "%")->orderBy('id')->get() : collect();
        
        // Buat array bimbingan per bagian
        $bimbingan_per_bagian = [];
        foreach ($bagians as $bagian) {
            $bimbingan = $mahasiswa->bimbingans()->where('bagian_id', $bagian->id)->first();
            $bimbingan_per_bagian[] = [
                'bagian' => $bagian,
                'bimbingan' => $bimbingan,
            ];
    ...
    {
        return view('pages.mahasiswa.bimbingan.create-offline', [
    ...
    public function storeOffline(Request $request)
    {
        ...
        return redirect('bimbingan-mahasiswa')->with('success', 'Bimbingan offline berhasil disubmit. Menunggu verifikasi dari prodi.');
    }
    ...
    public function detailOffline($id)
    {
        ...
        return view('pages.mahasiswa.bimbingan.detail-offline', [
    ...
    public function submitAccManualStore(Request $request)
    {
        ...
        return redirect('bimbingan-mahasiswa')->with('success', 'Lembar acc bimbingan berhasil diupload. Silahkan tunggu validasi dari prodi.');
    }
