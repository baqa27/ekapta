...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="mb-3">
                <a href="{{ route('kp.seminar.dosen') }}" class="btn btn-secondary shadow">
                ...
                <div class="col-12">
                    <div class="card card-primary card-outline">
                        @if($review_seminar->dosen_status == 'penguji')
                            ...
                            @elseif ($review_seminar->status == 'revisi')
                            bg-warning
                            @elseif ($review_seminar->status == 'diterima')
                            bg-success
                            @elseif ($review_seminar->status == 'ditolak')
                            bg-danger @endif
                            ">
                        ...
                        <div class="card-body">
                            <table>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $review_seminar->seminar->mahasiswa->prodi }}
                                    </td>
                                </tr>
                                <tr>
                                    <td><b class="mr-3">Judul KP</b></td>
                                    <td>{{ $review_seminar->seminar->pengajuan->judul }}
                                    </td>
                                </tr>
                            </table>
                            <hr>
                            ...
                            <div class="mt-4 text-secondary"><i class="fas fa-calendar mr-2"></i>
                                Tanggal Submit <b>{{ date('d M Y H:m', strtotime($review_seminar->created_at)) }}</b>
                            </div>

                            @if ($review_seminar->tanggal_acc)
                                <div class="text-success"><i class="fas fa-calendar-check mr-2"></i>
                                    Tanggal Acc <b>{{ date('d M Y H:m', strtotime($review_seminar->tanggal_acc)) }}</b>
                                </div>
                                <hr>
                            @endif
                            ...
                            <div class="p-2 rounded" style="background-color: #dbdbdb">
                                {!! nl2br($review_seminar->keterangan) !!}
                            </div>
                            <hr>

                            <div class="shadow-lg p-2 rounded">
                                <b>Laporan Seminar KP : </b>
                        ...
                        @if ($review_seminar->dosen_status == 'penguji' && $review_seminar->status == 'review')
                            <div class="card-footer">
                                <div class="d-flex">
                                    ...
                                    {{--<div onclick="confirmAcc()">
                                        <form action="{{ route('kp.review.seminar.acc') }}" method="post">
                                            @csrf
                    ...
                    @if($review_seminar->status == 'diterima' || $review_seminar->dosen_status == 'pembimbing')
                        <div class="card card-primary card-outline mt-2">
                            <div class="card-header">Input Nilai</div>
                            <div class="card-body">
                                <form action="{{ route('kp.review.seminar.nilai') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $review_seminar->id }}">
                                    <div class="row">
                                        <div class="col-md-3">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label for="exampleInputFile">Metodologi dan Redaksi KP</label>
                        ...
                        <div class="card-header">
                            <h3 class="card-title"><strong>Revisi</strong>
                                <span class="badge bg-danger rounded-pill">
                                    {{ count($review_seminar->revisis) }}
                                </span>
                            </h3>
                            @if ($review_seminar->status == 'revisi')
                                        ...
                                        <div class="float-right">
                                            <div class="d-flex">
                                                ...
                                                <div onclick="confirmDelete()">
                                                    <form action="{{ route('kp.review.seminar.revisi.delete') }}"
                                                        method="post">
                                                        @csrf
    ...
    <div class="modal fade" id="modal-revisi">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('kp.review.seminar.revisi.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $review_seminar->id }}">
                    <div class="modal-header">
                        <h4 class="modal-title">Revisi Seminar KP</h4>
    ...
    <div class="modal fade" id="modal-acc">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('kp.review.seminar.acc') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $review_seminar->id }}">
                    <div class="modal-header">
                        <h4 class="modal-title">Acc Seminar KP</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
