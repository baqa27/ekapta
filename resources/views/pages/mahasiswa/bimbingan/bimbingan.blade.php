    ...
    <div class="content">
        <div class="container">
            @if (\App\Helpers\AppHelper::check_bimbingan_is_complete($mahasiswa))
                <a href="{{ route('cetak.riwayat.bimbingan.mahasiswa') }}" class="btn btn-success mb-3"
            ...
            @if ($is_expired)
                <div class="mb-3 bg-danger rounded p-2 text-white">
                    Masa bimbingan anda sudah habis, silahkan lakukan <a
                        href="{{ route('pendaftaran.disable', $pendaftaran_acc->id) }}" class="text-white"><u><b>Perpanjangan KP!</b></u></a>
                </div>
            @else
                                        ...
                                        @if (!$is_expired)
                                            @if ($bimbingan && $bimbingan->status)
                                                ...
                                                @elseif ($bimbingan->status == 'revisi')
                                                    ...
                                                    <a href="{{ url('/bimbingan/edit/' . $bimbingan->id) }}" class="btn btn-success btn-sm shadow">
                                                ...
                                                @endif
                                            @else
                                                {{-- Belum submit - cek mode dosen --}}
                                                @php
                                                    $canSubmit = false;
                                                    if ($no - 1 == 1) {
                                                        $canSubmit = true;
                                                    } elseif (count(\App\Helpers\AppHelper::instance()->getBimbinganIsAcc($mahasiswa->id)) >= 1) {
                                                        if (!\App\Helpers\AppHelper::instance()->cekBagianIsAcc($mahasiswa->nim)) {
                                                            $canSubmit = true;
                                                        }
                                                    }
                                                @endphp
                                                ...
                                                @if ($canSubmit)
                                                    @if ($dosen_utama && $dosen_utama->is_manual)
                                                        ...
                                                        <a href="{{ route('bimbingan.create.manual') }}?bagian_id={{ $bagian->id }}" class="btn btn-warning btn-sm shadow">
                                                            <i class="fas fa-upload mr-1"></i> Input Acc Manual
                                                        </a>
                                                    @else
                                                        {{-- Dosen online: submit file ke sistem --}}
                                                        <a href="{{ route('bimbingan.create') }}?bagian_id={{ $bagian->id }}" class="btn btn-success btn-sm shadow">
