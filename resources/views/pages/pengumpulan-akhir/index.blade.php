...
@section('content')
    ...
    <div class="content">
        <div class="container">
            ...
            @if (count($jilids) == 0)
                <a href="{{ route('pengumpulan-akhir.create') }}" class="btn btn-primary mb-4"><i class="fas fa-plus mr-2"></i>
                    Ajukan Pengumpulan Akhir KP</a>
            @endif

            @if ($jilid)
                @if ($jilid->status == 3)
                    <div class="alert alert-primary">
                        <i class="fas fa-info-circle"></i> Dokumen Kerja Praktik sudah dikonfirmasi oleh admin.
                        Silahkan konfirmasi dan melakukan pembayaran dengan membawa dokumen-dokumen asli.
                    </div>
                @elseif ($jilid->status == 4)
                    <a href="#" class="btn btn-success shadow mb-3"><i class="bi bi-check-circle "></i>
                        PENGUMPULAN AKHIR SUDAH SELESAI DAN DIVERIFIKASI
                        <b>Rp {{ number_format($jilid->total_pembayaran, 0, ',', '.') }}</b></b></a>
                @elseif ($jilid->status == 2)
                    <a href="{{ route('pengumpulan-akhir.edit', $jilid->id) }}" class="btn btn-primary shadow mb-3"><i
                            class="bi bi-info-circle"></i> PENGUMPULAN AKHIR BERSTATUS REVISI. SILAHKAN SUBMIT ULANG</b></a>
                @elseif ($jilid->status == 1)
                    <a href="#" class="btn btn-secondary shadow mb-3"><i class="bi bi-hourglass-bottom "></i> SUDAH
                        MENGAJUKAN PENGUMPULAN AKHIR. SILAHKAN
                ...
                @endif
            @else
                <a href="{{ route('pengumpulan-akhir.create') }}" class="btn btn-primary shadow mb-3"><i class="fas fa-book"></i> AJUKAN
                    PENGUMPULAN AKHIR KP</a>
            @endif
                                ...
                                <tbody>
                                    ...
                                    @foreach ($jilids as $jilid)
                                        <tr>
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('pengumpulan-akhir.detail.mahasiswa', $jilid->id) }}">{{ $jilid->mahasiswa->pengajuans()->where('status', 'diterima')->first()->judul }}</a>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($jilid->status == 2)
                                                    <a href="{{ route('pengumpulan-akhir.edit', $jilid->id) }}"
                                                        class="btn btn-primary btn-sm"><i class="fas fa-upload"></i>
                                                        Submit</a>
                                                @else
                                                    <a href="{{ route('pengumpulan-akhir.detail.mahasiswa', $jilid->id) }}"
